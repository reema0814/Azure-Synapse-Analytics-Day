...
2. Select `Azure Purview` to view the details of the connected Azure Purview workspace
    ...
    ![Azure Purview connection in Azure Synapse Analytics workspace](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-synapse-purview-connection.png)
...
2. In the list of pipelines, select the `Import WWI Data - Fact Sale Full` pipeline
    ...
    ![Select the Import WWI Data - Fact Sale Full pipeline](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-select-synapse-pipeline.png)
...
3. Select `Add trigger` and then select `Trigger now` to initiate the execution of the pipeline. Select `OK` in the `Pipeline run` popup to activate the trigger.
    ...
    ![Trigger the Synapse pipeline execution](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-trigger-synapse-pipeline.png)
...
4. In Synapse Studio, navigate to the `Monitor` hub and then select `Pipeline runs`. The `Import WWI Data - Fact Sale Full` pipeline should appear with a status of `In progress`.
    ...
    ![Ensure Synapse pipeline is successfuly triggered](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-validate-pipeline-execution.png)
...
2. Select the search bar at the top and make sure `Purview` is selected as the search provider. Enter `fact` in the search box and notice the asset suggestions returned. Select `View search results` to view the results of the Purview-powered search.
    ...
    ![View Purview-powered search results](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-search-in-synapse-workspace.png)

3. Explore the results of the search. Select the `FactSale` asset.

    ![Explore Purview-powered search results](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-search-results-1.png)

4. Explore the properties of the `FactSale` asset.

    ![Explore Purivew-powered searchr result - continued](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-search-results-2.png)
...
1. In the Azure Portal, navigate to the resource group that contains your Synapse Analytics workspace. Select the Purview workspace object (which should be named `purviewworkspaceNNNNNN`) and then select `Open Purview Studio`.
    ...
    ![Open Purview Studio](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-open-purview-studio.png)
...
2. In Purview Studio, navigate to the `Data map` hub, select `Sources` and then locate the Synapse Analytics source which is your Synapse Analytics workspace. Select `View details` to explore the configuration of the source.
    ...
    ![Locate Synpse Analytics source in Purview Studio](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-synapse-analytics-source.png)
...
3. Note the details of the source:
    ...
    ![Synapse Analytics source details in Purview Studio](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-synapse-analytics-source-details-1.png)

4. Select `Edit source` to view the details of the connection to the Synapse Analytics workspace.

    ![Synapse Analytics source connection properties](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-synapse-analytics-source-details-2.png)
...
5. In Purview Studio, navigate to the `Data map` hub, select `Collections` and then select the root collection (which should be named `purviewworkspaceNNNNNN` just like the Purview workspace). Select `Assets` to explore the list of assets available in the collection.
    ...
    ![Purview collection](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-collection.png)

6. Observe the list of assets in the collection and their source types.

    ![Purview collection assets](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-collection-assets.png)
...
1. In the list of Purview collection assets, note the `Copy data1` asset of type Azure Synapse Analytics.
    ...
    ![Synapse Pipeline asset in Purview collection](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-collection-assets-synapse-pipeline.png)
...
2. Select the asset named `Copy data1` to view its details.
    ...
    ![Synapse Pipeline asset details](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-collection-assets-synapse-pipeline-details.png)

3. Select the `Lineage` tab to view the detailed lineage data.

    ![Synapse Pipeline asset details lineage information](https://github.com/solliancenet/azure-synapse-analytics-day/blob/master/media/ex06-purview-collection-assets-synapse-pipeline-details-lineage.png)
