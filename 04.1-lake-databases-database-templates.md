...
9. Select the `Direct` option from drop-down as the **Mapping method**, select `customer@CustomerId` as the **Source column** value and select `CustomerId` as the **Target column** field value for the first mapping.
   ...
   >**Note**: If the data is already populated. Please skip to step 11.
...
12. You will able to see the preview data available on the Target Customer table.
   
    ![Results of data mapping](./media/mapping4.png)
   
13. Click on **Create pipeline** under CustomerMapping.

    ![Create pipeline data mapping](./media/ex4-t3-create-pipeline.png)
   
14. You have generated the Pipeline with the Data Flow activities and the mapping between the source & target table automatically.
...
15. Click the **Debug** button of the `CustomerMapping_StartingPipeline` pipeline to trigger the Data flow from source to target table.

    ![Debug pipeline data mapping](./media/ex4-t3-debug-result.png)

16. In Synapse Studio, navigate to the `Develop` hub and create a new SQL script. Make sure the `Built-in` serverless SQL pool is selected as well as the `Database1` database.
