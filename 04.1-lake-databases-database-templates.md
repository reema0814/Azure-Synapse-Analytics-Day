...
7. Select **Customer** table under Database1 and click on **New Mapping** which will display the **Mapping Method**, **Source column** and **Target Column** for mapping between the Source & Target Table.
   ...
   ![New data mapping](./media/ex4-t3-customermapping.png)
...
8. Select the `Direct` option from drop-down as the **Mapping method**, select `customer@CustomerId` as the **Source column** value and select `CustomerId` as the **Target column** field value for the first mapping.
   ...
   ![Adding new data mapping](./media/ex4-t3-map1.png)

9. Click the New Mapping button again. It will create a New Mapping option next to the First mapping that we have mapped in the previous step.

   - Select `Direct` as Mapping Method option, `customer@FirstName` as the Source column field value and `FirstName` as the Target column field value. 

10. Click the New Mapping button again. It will create a New Mapping option next to the second mapping we have mapped above. 

   - Select `Direct` as the Mapping Method option, `customer@LastName` as the Source column field value and `LastName` as the Target column field value.

   ![Added data mapping](./media/ex4-t3-mapping-refresh.png)

11. To preview the data available on the Target Customer table, select **Refresh** under **Data Preview**.
   
    ![Results of data mapping](./media/ex4-t3-mapping-results.png)
   
12. Click on **Create pipeline** under CustomerMapping.

    ![Create pipeline data mapping](./media/ex4-t3-create-pipeline.png)
   
13. You have generated the Pipeline with the Data Flow activities and the mapping between the source & target table automatically.

    ![Created pipeline data mapping](./media/ex4-t3-create-pipeline.png)
